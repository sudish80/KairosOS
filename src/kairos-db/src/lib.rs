...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let storage =
            std::sync::Arc::new(storage::StorageEngine::new(std::sync::Arc::clone(&config)).await?);
        let vector_db =
            std::sync::Arc::new(vector::VectorDatabase::new(std::sync::Arc::clone(&config)));
        let query_engine = std::sync::Arc::new(query::QueryEngine::new(
            std::sync::Arc::clone(&config),
            std::sync::Arc::clone(&vector_db),
        ));
