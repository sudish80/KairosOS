...
impl MemoryBus {
    ...
    pub async fn publish(&self, _channel: &str, _msg: &str) -> anyhow::Result<()> { self.messages.write().await.push(_msg.to_string()); Ok(()) }
    pub async fn subscribe(&self, _channels: &[&str]) -> Vec<String> { self.messages.write().await.drain(..).collect() }
}
