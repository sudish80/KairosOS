...
pub mod market; pub mod trading; pub mod risk;
pub struct AppState {
...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let trading = std::sync::Arc::new(trading::TradingEngine::new(std::sync::Arc::clone(&config), std::sync::Arc::clone(&market)));
        let risk = std::sync::Arc::new(risk::RiskManager::new(std::sync::Arc::clone(&config)));
        tracing::info!("kairos-finance AppState initialized");
        Ok(Self { config, telemetry, market, trading, risk })
    }
}
