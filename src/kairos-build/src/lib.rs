...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let builder =
            std::sync::Arc::new(builder::ImageBuilder::new(std::sync::Arc::clone(&config)));
        let image_manager =
            std::sync::Arc::new(image::ImageManager::new(std::sync::Arc::clone(&config)));
        let package_manager =
            std::sync::Arc::new(package::PackageManager::new(std::sync::Arc::clone(&config)));
