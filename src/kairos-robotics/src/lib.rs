...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let m = std::sync::Arc::new(control::MotorController::new(std::sync::Arc::clone(&c)));
        let k = std::sync::Arc::new(kinematics::KinematicsEngine::new(std::sync::Arc::clone(&c)));
        tracing::info!("kairos-robotics AppState initialized");
        Ok(Self {
            config: c,
            telemetry: t,
            controller: m,
