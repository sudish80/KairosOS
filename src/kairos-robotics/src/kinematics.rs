...
impl KinematicsEngine {
    ...
    pub fn forward(&self, _joint_angles: &[f64]) -> Vec<f64> { vec![0.0; 3] }
    pub fn inverse(&self, _target: &[f64]) -> Vec<f64> { vec![0.0; self.config.blocking_read().kinematics.num_joints as usize] }
}
