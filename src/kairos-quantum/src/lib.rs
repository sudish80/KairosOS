...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let g = std::sync::Arc::new(gate::GateEngine::new(std::sync::Arc::clone(&c)));
        let s = std::sync::Arc::new(simulator::QuantumSimulator::new(std::sync::Arc::clone(&c)));
        tracing::info!("kairos-quantum AppState initialized");
        Ok(Self {
            config: c,
            telemetry: t,
            gate_engine: g,
