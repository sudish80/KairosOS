...
#[tokio::main] async fn main() -> anyhow::Result<()> {
    let cli=Cli::parse(); let cfg=kairos_quantum::config::Config::load(&cli.config)?;
    ...
    let state=kairos_quantum::AppState::new(cfg).await?; let wk=kairos_quantum::worker::QuantumWorker::new(state.config.clone()); wk.start().await?;
