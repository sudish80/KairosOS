...
#[derive(Parser)]
#[command(name = "kairos-bpf", version = "1.0.0", about = "eBPF telemetry, anomaly detection, and autonomous remediation daemon")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    let cfg = if cli.config.exists() {
        ...
        info!("Config not found at {}, using defaults", cli.config.display());
    ...
    info!("kairos-bpf v{} starting (pid={})", env!("CARGO_PKG_VERSION"), std::process::id());
