...
impl PluginEngine {
                ...
                if manifest_path.exists() {
                    match self.load_manifest(&manifest_path).await {
                        Ok(manifest) => {
                            ...
                            self.plugins.write().await.insert(info.manifest.name.clone(), info);
                        }
                        Err(e) => warn!("Failed to load plugin manifest at {:?}: {}", manifest_path, e),
                    }
                }
            }
        }
    ...
    pub async fn trigger_event(&self, event: &str, payload: serde_json::Value) -> anyhow::Result<()> {
        let links = self.links.read().await;
        let relevant: Vec<_> = links.iter()
        ...
        for link in relevant {
            ...
            self.execute_plugin(&link.source, "handle_event", serde_json::json!({
                "event": event,
                "payload": payload,
            })).await?;
        }
    ...
    pub async fn execute_plugin(&self, name: &str, method: &str, params: serde_json::Value) -> anyhow::Result<serde_json::Value> {
        let plugin = {
    ...
    async fn run_wasm(&self, wasm_path: &Path, method: &str, params: &serde_json::Value) -> anyhow::Result<serde_json::Value> {
        ...
        linker.func_wrap("env", "read_args", |mut caller: wasmtime::Caller<'_, ()>, ptr: i32, len: i32| {
            let mem = match caller.get_export("memory") {
                Some(wasmtime::Extern::Memory(m)) => m,
                _ => return Err(wasmtime::Trap::new("no memory export")),
            };
        ...
        if result_ptr <= 0 {
            return Err(anyhow::anyhow!("WASM plugin returned error code {}", result_ptr));
        }
        ...
        let result: serde_json::Value = serde_json::from_slice(&result_data)
            .unwrap_or(serde_json::json!({"output": String::from_utf8_lossy(&result_data).to_string()}));
...
mod tests {
    ...
    async fn test_plugin_manifest_loading() {
        ...
        fs::write(&manifest_path, toml::to_string(&manifest).unwrap()).await.unwrap();
