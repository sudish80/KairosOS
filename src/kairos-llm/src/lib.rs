...
pub mod runtime; pub mod quantizer; pub mod model;
pub struct AppState {
...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let model_registry = std::sync::Arc::new(model::ModelRegistry::new(std::sync::Arc::clone(&config)));
        let quantizer = std::sync::Arc::new(quantizer::Quantizer::new(std::sync::Arc::clone(&config)));
        let runtime = std::sync::Arc::new(runtime::LlmRuntime::new(std::sync::Arc::clone(&config), std::sync::Arc::clone(&model_registry)));
        tracing::info!("kairos-llm AppState initialized");
        Ok(Self { config, telemetry, runtime, quantizer, model_registry })
    }
}
