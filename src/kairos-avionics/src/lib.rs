...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let a = std::sync::Arc::new(arinc::Arinc429::new(std::sync::Arc::clone(&c)));
        let m = std::sync::Arc::new(mavlink::MavlinkBus::new(std::sync::Arc::clone(&c)));
        tracing::info!("kairos-avionics AppState initialized");
        Ok(Self {
            config: c,
            telemetry: t,
            arinc: a,
