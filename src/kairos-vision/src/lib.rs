...
pub mod capture; pub mod detect; pub mod pipeline;
pub struct AppState {
...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let capture = std::sync::Arc::new(capture::FrameCapture::new(std::sync::Arc::clone(&config)));
        let detector = std::sync::Arc::new(detect::ObjectDetector::new(std::sync::Arc::clone(&config)));
        let vision_pipeline = std::sync::Arc::new(pipeline::VisionPipeline::new(std::sync::Arc::clone(&config), std::sync::Arc::clone(&capture), std::sync::Arc::clone(&detector)));
        tracing::info!("kairos-vision AppState initialized");
        Ok(Self { config, telemetry, capture, detector, vision_pipeline })
    }
}
