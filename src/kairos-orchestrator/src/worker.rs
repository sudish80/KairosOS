...
impl OrchestratorWorker {
    pub fn new(config: Arc<RwLock<config::Config>>, executor: Arc<TaskExecutor>) -> Self { Self { config, executor } }
    pub async fn start(&self) -> anyhow::Result<()> {
        info!("OrchestratorWorker started");
        tokio::spawn(async move { loop { tokio::time::sleep(std::time::Duration::from_secs(60)).await; } });
        Ok(())
    }
}
