...
impl RepoManager {
    pub async fn new(config: Arc<RwLock<config::Config>>) -> anyhow::Result<Self> {
        ...
        Ok(Self { config, bare_path, workdir, git_path: bare_path.clone() })
    }
    ...
    pub async fn add_and_commit(&self, paths: &[PathBuf], message: &str) -> anyhow::Result<String> {
        ...
        if !temp_clone.exists() {
            Command::new("git")
                .args(["clone", &self.bare_path.to_string_lossy(), &temp_clone.to_string_lossy()])
                .status().await?;
        }
        ...
        let output = Command::new("git")
            .args([
                "-C", &temp_clone.to_string_lossy(),
                "-c", format!("user.name={}", cfg.commit.author_name),
                "-c", format!("user.email={}", cfg.commit.author_email),
                "commit", "-m", message,
                "--allow-empty",
            ])
            .output().await?;
        ...
        // Push to bare repo
        Command::new("git")
            .args(["-C", &temp_clone.to_string_lossy(), "push", "origin", "HEAD"])
            .status().await?;
    ...
    pub async fn get_log(&self, max_count: usize) -> anyhow::Result<Vec<CommitInfo>> {
        let output = Command::new("git")
            .args([
                "--git-dir", &self.bare_path.to_string_lossy(),
                "log", format!("--max-count={}", max_count).as_str(),
                "--format=%H|%ai|%s",
            ])
            .output().await?;
    ...
    pub async fn get_head_hash(&self) -> anyhow::Result<Option<String>> {
        let output = Command::new("git")
            .args(["--git-dir", &self.bare_path.to_string_lossy(), "rev-parse", "HEAD"])
            .output().await?;
    ...
    pub async fn run_gc(&self) -> anyhow::Result<()> {
        Command::new("git")
            .args(["--git-dir", &self.bare_path.to_string_lossy(), "gc", "--auto", "--quiet"])
            .status().await?;
