...
impl WireGuardManager {
    ...
    pub async fn bring_up(&self) -> anyhow::Result<()> {
        ...
        if status.success() {
            info!("WireGuard {} up", iface);
            Ok(())
        } else {
            Err(anyhow::anyhow!("Failed to bring up {}", iface))
        }
    }
    pub async fn bring_down(&self) -> anyhow::Result<()> {
        ...
        Command::new(WG_QUICK)
            ...
            .status()
            .await?;
        info!("WireGuard {} down", iface);
        Ok(())
    }
    pub async fn add_peer(&self, peer: &str) -> anyhow::Result<()> {
        let iface = self.config.read().await.wg.interface.clone();
        Command::new("wg")
            .args(["set", &iface, "peer", peer, "allowed-ips", "0.0.0.0/0"])
            .status()
            .await?;
