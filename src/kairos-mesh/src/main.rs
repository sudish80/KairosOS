...
#[command(
    name = "kairos-mesh",
    version = "1.0.0",
    about = "WireGuard mesh networking and consensus daemon"
)]
struct Cli {
    #[arg(short, long, default_value = "/etc/kairos/mesh.toml")]
    config: PathBuf,
    #[arg(long)]
    up: bool,
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let cfg = kairos_mesh::config::Config::load(&cli.config)?;
    ...
    if cli.down {
        state.wg_manager.bring_down().await?;
        return Ok(());
    }
    state.wg_manager.bring_up().await?;
