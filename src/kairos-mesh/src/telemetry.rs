...
impl Telemetry {
    ...
    pub fn record_discovery(&self) {
        self.discovery_events.fetch_add(1, Ordering::Relaxed);
    }
    pub fn record_error(&self) {
        self.errors_total.fetch_add(1, Ordering::Relaxed);
    }
    pub fn metrics(&self) -> serde_json::Value {
        serde_json::json!({"peers_connected": self.peers_connected.load(Ordering::Relaxed), "bytes_sent": self.bytes_sent.load(Ordering::Relaxed), "bytes_recv": self.bytes_recv.load(Ordering::Relaxed), "discovery_events": self.discovery_events.load(Ordering::Relaxed), "errors_total": self.errors_total.load(Ordering::Relaxed)})
    }
}
