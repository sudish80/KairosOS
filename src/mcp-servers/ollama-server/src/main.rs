...
#[derive(Parser)]
#[command(name = "kairos-ollama-mcp", version = "1.0.0", about = "Ollama MCP server — LLM inference via Ollama API over JSON-RPC 2.0")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt().with_env_filter(EnvFilter::from_default_env().add_directive("ollama_mcp_server=info".parse()?)).init();
    let _cli = Cli::parse();
