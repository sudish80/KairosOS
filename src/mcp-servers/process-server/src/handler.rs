...
impl ProcessHandler {
    ...
    async fn handle_list(&self) -> Result<serde_json::Value, McpError> {
        ...
        let output = Command::new(&cfg.ps_path)
            .args(["axo", "pid,ppid,user,%cpu,%mem,rss,stat,args", "--no-headers"])
            .output().await?;
        if !output.status.success() {
            return Err(McpError::Internal(format!("ps exited with {}", output.status)));
        }
        ...
        for line in stdout.lines() {
            ...
            let parts: Vec<&str> = trimmed.splitn(8, char::is_whitespace).filter(|s| !s.is_empty()).collect();
            if parts.len() < 7 { continue; }
            let command = if parts.len() > 7 { parts[7..].join(" ") } else { String::new() };
            ...
            if processes.len() >= cfg.max_processes_returned { break; }
        }
    ...
    async fn handle_signal(&self, req: &serde_json::Value) -> Result<serde_json::Value, McpError> {
        let cfg = self.config.read().await;
        let pid = req["params"]["pid"].as_u64().ok_or(McpError::InvalidParams("missing pid".into()))?;
        let sig = req["params"]["signal"].as_str().unwrap_or("TERM");
        if !cfg.allowed_signals.iter().any(|s| s.eq_ignore_ascii_case(sig)) {
            return Err(McpError::SignalNotAllowed(sig.into()));
        }
        let output = Command::new(&cfg.kill_path).args(["-s", sig, &pid.to_string()]).output().await?;
        if !output.status.success() {
