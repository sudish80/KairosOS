...
#[derive(Parser)]
#[command(name = "kairos-process-mcp", version = "1.0.0", about = "Process MCP server — list, signal, monitor processes via JSON-RPC 2.0")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt().with_env_filter(EnvFilter::from_default_env().add_directive("process_mcp_server=info".parse()?)).init();
    let _cli = Cli::parse();
