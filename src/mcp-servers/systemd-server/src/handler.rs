...
impl SystemdHandler {
    ...
    pub async fn handle_request(&self, req: &serde_json::Value) -> serde_json::Value {
        ...
        let result = match method {
            ...
            "start_service" | "stop_service" | "restart_service" | "enable_service"
            | "disable_service" => {
    ...
    async fn handle_status(&self, req: &serde_json::Value) -> Result<serde_json::Value, McpError> {
        ...
        Ok(
            serde_json::json!({ "output": stdout, "stderr": stderr, "exit_code": output.status.code().unwrap_or(-1) }),
        )
    }
    ...
    async fn handle_action(
        ...
        let stderr = String::from_utf8(output.stderr)?;
        if !output.status.success() {
            return Err(McpError::ServiceActionFailed(format!(
                "{} {}: {}",
                action, name, stderr
            )));
        }
