...
#[derive(Parser)]
#[command(name = "kairos-systemd-mcp", version = "1.0.0", about = "Systemd MCP server — manage services and journal via JSON-RPC 2.0")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt().with_env_filter(EnvFilter::from_default_env().add_directive("systemd_mcp_server=info".parse()?)).init();
    let _cli = Cli::parse();
