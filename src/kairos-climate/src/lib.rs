...
impl AppState {
    pub async fn new(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let assimilation = std::sync::Arc::new(assimilation::DataAssimilation::new(
            std::sync::Arc::clone(&config),
        ));
        let model_engine =
            std::sync::Arc::new(model::ClimateModel::new(std::sync::Arc::clone(&config)));
        let forecast_engine = std::sync::Arc::new(forecast::ForecastEngine::new(
            std::sync::Arc::clone(&config),
            std::sync::Arc::clone(&model_engine),
        ));
